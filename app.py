...
INTERACTIVE_ROUNDS_PER_LEVEL = 3
...
<head>
  ...
  <style>
    ...
    .board {
      ...
      gap: 8px;
      --board-size: 1;
      grid-template-columns: repeat(var(--board-size), minmax(46px, 1fr));
...
<body>
  <div class=\"app-shell\">
    <aside class=\"panel\">
      ...
      <div class=\"actions\">
        ...
        <button type=\"button\" class=\"secondary\" id=\"show-data\">Show Data</button>
    ...
    <main class=\"panel game-surface\">
      <div class=\"board-wrapper\">
        <div class=\"board-meta\">
          ...
          <div><strong>Target interactive rounds:</strong> {{ rounds }}</div>
        </div>
  ...
  <script>
    (function() {
      ...
      const defaultLevels = {{ levels | tojson }};
      const targetInteractiveRounds = {{ rounds }};
      ...
      function updateBoardMarkers() {
        ...
        state.qubitButtons.forEach((btn, idx) => {
          btn.classList.toggle('has-error', Boolean(error[idx]));
          btn.classList.toggle('corrected', Boolean(correction[idx]));
          btn.classList.toggle('residual', Boolean(residual[idx]));
        });
      ...
      function startGame() {
        ...
        addLog('New session ' + state.sessionId + ' started with d=' + distance + '.');
      ...
      function finishLevel() {
        state.levelsData.push({
          ...
          logical_errors: state.levelStats.logicalErrors,
      ...
      function finishRun() {
        ...
        const totals = state.levelsData.reduce((acc, level) => {
          acc.logical_errors += level.logical_errors;
          acc.interactive_rounds += level.interactive_rounds;
          acc.no_syndrome_rounds += level.no_syndrome_rounds;
          return acc;
        }, { logical_errors: 0, interactive_rounds: 0, no_syndrome_rounds: 0 });
        ...
        const payload = {
          ...
          successful_rounds_per_level: { levels: summary.levels, totals: summary.totals },
          rounds_per_level: summary.rounds_per_level,
          level_reached: summary.levels.length,
          logical_errors: summary.totals.logical_errors,
        };
...
def index():
    ...
    return render_template_string(
        ...
        rounds=INTERACTIVE_ROUNDS_PER_LEVEL,
...
def api_game_save():
    ...
    rounds_per_level = payload.get("rounds_per_level", INTERACTIVE_ROUNDS_PER_LEVEL)
